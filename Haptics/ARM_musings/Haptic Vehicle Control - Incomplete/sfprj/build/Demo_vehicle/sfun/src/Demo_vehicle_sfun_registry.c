...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
 const mxArray * prhs[] )
{
  extern unsigned int sf_Demo_vehicle_process_check_sum_call( int nlhs, mxArray
   * plhs[], int nrhs, const mxArray * prhs[] );

  if(sf_Demo_vehicle_process_check_sum_call(nlhs,plhs,nrhs,prhs)) return 1;
  return 0;
}
unsigned int sf_process_autoinheritence_call( int nlhs, mxArray * plhs[], int
 nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_Demo_vehicle_autoinheritance_info( int nlhs, mxArray *
   plhs[], int nrhs, const mxArray * prhs[] );
  if(sf_Demo_vehicle_autoinheritance_info(nlhs,plhs,nrhs,prhs)) return 1;
  return 0;
}
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray
 * plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_Demo_vehicle_get_eml_resolved_functions_info( int nlhs,
  ...
  if(strcmp(machineName, "Demo_vehicle") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_Demo_vehicle_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_Demo_vehicle_method_dispatcher(SimStruct *S, unsigned int
...
{
  if(!strcmp(machineName,"Demo_vehicle")) {
    return(sf_Demo_vehicle_method_dispatcher(simstructPtr,chartFileNumber,method,data));
  }
  return 0;
}
extern void Demo_vehicle_terminator(void);
void sf_machine_global_terminator(void)
{
  if(sfMachineGlobalTerminatorCallable) {
    ...
    Demo_vehicle_terminator();
    sf_debug_terminate();
  }
  return;
}
extern void Demo_vehicle_initializer(void);
extern void Demo_vehicle_register_exported_symbols(SimStruct* S);
extern void Demo_vehicle_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  Demo_vehicle_register_exported_symbols(S);
}
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if(sfMachineGlobalInitializerCallable) {
    ...
    if(!simModeIsRTWGen) {
      Demo_vehicle_debug_initialize();
    }
    Demo_vehicle_initializer();
  }
  return;
}
