...
typedef Demo_vehicle_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
extern RT_MODEL * Demo_vehicle(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Demo_vehicle_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://Demo_vehicle:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Demo_vehicle -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
