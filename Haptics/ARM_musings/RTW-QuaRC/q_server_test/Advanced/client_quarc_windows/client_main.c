...
typedef client_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
extern RT_MODEL * client(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(client_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  {
    stream_shutdown(client_DWork.StreamConnect_Stream);
  }
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://client:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: client -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
