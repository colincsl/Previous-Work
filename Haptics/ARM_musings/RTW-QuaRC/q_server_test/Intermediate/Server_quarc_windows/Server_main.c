...
typedef Server_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
extern RT_MODEL * Server(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Server_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  {
    stream_shutdown(Server_DWork.StreamAnswer_Client);
    stream_shutdown(Server_DWork.StreamAnswer_Listener);
  }
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://Server:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Server -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
