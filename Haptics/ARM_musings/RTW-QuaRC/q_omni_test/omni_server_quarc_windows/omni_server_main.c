...
typedef omni_server_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
extern RT_MODEL * omni_server(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(omni_server_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  {
    stream_shutdown(omni_server_DWork.StreamCall_Stream);
  }
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://omni_server:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: omni_server -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
