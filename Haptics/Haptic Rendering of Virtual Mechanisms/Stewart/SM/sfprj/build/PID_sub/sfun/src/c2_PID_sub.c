...
#include "PID_sub_sfun_debug_macros.h"
...
static SFc2_PID_subInstanceStruct chartInstance;

/* Function Declarations */
static void initialize_c2_PID_sub(void);
static void initialize_params_c2_PID_sub(void);
static void enable_c2_PID_sub(void);
static void disable_c2_PID_sub(void);
static void ext_mode_exec_c2_PID_sub(void);
static void finalize_c2_PID_sub(void);
static void sf_c2_PID_sub(void);
...
static void initialize_c2_PID_sub(void)
{
  uint8_T *c2_is_active_c2_PID_sub;
  c2_is_active_c2_PID_sub = (uint8_T *)ssGetDWork(chartInstance.S, 1);
  _sfTime_ = (real_T)ssGetT(chartInstance.S);
  *c2_is_active_c2_PID_sub = 0U;
}
...
static void sf_c2_PID_sub(void)
{
  ...
  *c2_b_o6 = c2_o6;
  ...
  sf_debug_check_for_state_inconsistency(_PID_subMachineNumber_,
...
const mxArray *sf_c2_PID_sub_get_eml_resolved_functions_info(void)
{
...
/* SFunction Glue Code */
void sf_c2_PID_sub_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c2_PID_sub_get_autoinheritance_info(void)
{
 ...
 fullDebuggerInitialization)
{
  if(ssIsFirstInitCond(S) && fullDebuggerInitialization==1) {
    ...
    if(!sim_mode_is_rtw_gen(S)) {
      {
        ...
        chartAlreadyPresent = sf_debug_initialize_chart(_PID_subMachineNumber_,
         2,
         1,
         1,
         13,
        ...
        if(chartAlreadyPresent==0) {
          ...
          init_script_number_translation(_PID_subMachineNumber_,chartInstance.chartNumber);
          sf_debug_set_chart_disable_implicit_casting(_PID_subMachineNumber_,chartInstance.chartNumber,1);
          sf_debug_set_chart_event_thresholds(_PID_subMachineNumber_,
           chartInstance.chartNumber,
           0,
           0,
           0);
    ...
    sf_debug_reset_current_state_configuration(_PID_subMachineNumber_,chartInstance.chartNumber,chartInstance.instanceNumber);
  }
}

static void sf_opaque_initialize_c2_PID_sub(void *chartInstanceVar)
{
  chart_debug_initialization(chartInstance.S,0);
  initialize_params_c2_PID_sub();
  initialize_c2_PID_sub();
}

static void sf_opaque_enable_c2_PID_sub(void *chartInstanceVar)
{
  enable_c2_PID_sub();
}

static void sf_opaque_disable_c2_PID_sub(void *chartInstanceVar)
{
  disable_c2_PID_sub();
}

static void sf_opaque_gateway_c2_PID_sub(void *chartInstanceVar)
{
  sf_c2_PID_sub();
}

static void sf_opaque_ext_mode_exec_c2_PID_sub(void *chartInstanceVar)
{
  ext_mode_exec_c2_PID_sub();
}

static void sf_opaque_terminate_c2_PID_sub(void *chartInstanceVar)
{
...
static void mdlProcessParameters_c2_PID_sub(SimStruct *S)
{
  int i;
...
static void mdlSetWorkWidths_c2_PID_sub(SimStruct *S)
{
  if(sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable("PID_sub",2);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop("PID_sub",2,"gatewayCannotBeInlinedMultipleTimes"));
    if(chartIsInlinable) {
      ...
      sf_mark_chart_expressionable_inputs(S,"PID_sub",2,7);
      sf_mark_chart_reusable_outputs(S,"PID_sub",2,6);
    }
    sf_set_rtw_dwork_info(S,"PID_sub",2);
...
static void mdlRTW_c2_PID_sub(SimStruct *S)
{
  if(sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "PID_sub", 2);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void mdlStart_c2_PID_sub(SimStruct *S)
{
  chartInstance.chartInfo.chartInstance = NULL;
  chartInstance.chartInfo.isEMLChart = 1;
  chartInstance.chartInfo.chartInitialized = 0;
  chartInstance.chartInfo.sFunctionGateway = sf_opaque_gateway_c2_PID_sub;
  chartInstance.chartInfo.initializeChart = sf_opaque_initialize_c2_PID_sub;
  chartInstance.chartInfo.terminateChart = sf_opaque_terminate_c2_PID_sub;
  chartInstance.chartInfo.enableChart = sf_opaque_enable_c2_PID_sub;
  chartInstance.chartInfo.disableChart = sf_opaque_disable_c2_PID_sub;
  chartInstance.chartInfo.zeroCrossings = NULL;
  chartInstance.chartInfo.outputs = NULL;
  chartInstance.chartInfo.derivatives = NULL;
  chartInstance.chartInfo.mdlRTW = mdlRTW_c2_PID_sub;
  chartInstance.chartInfo.mdlStart = mdlStart_c2_PID_sub;
  chartInstance.chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c2_PID_sub;
  chartInstance.chartInfo.extModeExec = sf_opaque_ext_mode_exec_c2_PID_sub;
...
void c2_PID_sub_method_dispatcher(SimStruct *S, int_T method, void *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c2_PID_sub(S);
    break;
   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c2_PID_sub(S);
    break;
   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c2_PID_sub(S);
    break;
   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
     "Error calling c2_PID_sub_method_dispatcher.\n"
