...
typedef double_w_haptics_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
extern RT_MODEL * double_w_haptics(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(double_w_haptics_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: double_w_haptics -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
