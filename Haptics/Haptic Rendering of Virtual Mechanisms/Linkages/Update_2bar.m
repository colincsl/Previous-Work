function hObject = Update_2bar(hObject, eventdata, handles)
%
...
%get info from gui
G_len = str2double(get(obj.Double_follow_len, 'String'));
R_len = str2double(get(obj.Double_base_len, 'String'));

B_M = str2double(get(obj.Double_base_M, 'String'));
Bone_M = str2double(get(obj.Double_follow_M, 'String'));
...
Bone_CS2_P = [G_P(1)-G_len, R_len, 0]; %left
Bone_CG_P =  [(G_P(1)-G_len)/2, R_len, 0];

len = ((R_len)^2+G_len^2)^.5;
ang = asin((G_P(1)-G_len)/(G_len));
% a = ang*180/pi;

Bone_CS3_P = [G_P(1)-G_len, R_len, 0];  %sensor+actuator
Bone_CS4_P = [G_P(1)-G_len, R_len, 0];
