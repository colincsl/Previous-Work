...
typedef vmrl_test_fourbar_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
extern RT_MODEL * vmrl_test_fourbar(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(vmrl_test_fourbar_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: vmrl_test_fourbar -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
