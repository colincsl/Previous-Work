...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
 const mxArray * prhs[] )
{
  extern unsigned int sf_Pendulum_process_check_sum_call( int nlhs, mxArray *
   plhs[], int nrhs, const mxArray * prhs[] );

  if(sf_Pendulum_process_check_sum_call(nlhs,plhs,nrhs,prhs)) return 1;
  return 0;
}
unsigned int sf_process_autoinheritence_call( int nlhs, mxArray * plhs[], int
 nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_Pendulum_autoinheritance_info( int nlhs, mxArray *
   plhs[], int nrhs, const mxArray * prhs[] );
  if(sf_Pendulum_autoinheritance_info(nlhs,plhs,nrhs,prhs)) return 1;
  return 0;
}
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray
 * plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_Pendulum_get_eml_resolved_functions_info( int nlhs,
  ...
  if(strcmp(machineName, "Pendulum") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_Pendulum_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_Pendulum_method_dispatcher(SimStruct *S, unsigned int
...
{
  if(!strcmp(machineName,"Pendulum")) {
    return(sf_Pendulum_method_dispatcher(simstructPtr,chartFileNumber,method,data));
  }
  return 0;
}
extern void Pendulum_terminator(void);
...
extern void Pendulum_initializer(void);
extern void Pendulum_register_exported_symbols(SimStruct* S);
extern void Pendulum_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  Pendulum_register_exported_symbols(S);
}
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if(sfMachineGlobalInitializerCallable) {
    ...
    if(!simModeIsRTWGen) {
      Pendulum_debug_initialize();
    }
    Pendulum_initializer();
  }
  return;
}
