...
typedef Pong_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
extern RT_MODEL * Pong(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Pong_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://Pong:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Pong -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      if (subrate_index < 0) {         /* all threads successfully created */
        ...
        fflush(stdout);
        result = hil_task_start(Pong_DWork.HILReadEncoderTimebase_Task,
          Pong_P.HILReadEncoderTimebase_Clock, 1000.0, -1)
          ;
        if (result == 0) {
          ...
          /* disarm the timebase */
          hil_task_stop(Pong_DWork.HILReadEncoderTimebase_Task)
            ;
