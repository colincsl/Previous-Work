...
typedef Force_Rendering_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
extern RT_MODEL * Force_Rendering(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Force_Rendering_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Force_Rendering -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(Force_Rendering_DWork.HILReadEncoderTimebase_Task,
        Force_Rendering_P.HILReadEncoderTimebase_Clock, 1000.0, -1)
        ;
      if (result == 0) {
        ...
        /* disarm the timebase */
        hil_task_stop(Force_Rendering_DWork.HILReadEncoderTimebase_Task)
          ;
