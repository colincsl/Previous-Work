...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
 const mxArray * prhs[] )
{
  extern unsigned int sf_Force_Rendering_process_check_sum_call( int nlhs,
   mxArray * plhs[], int nrhs, const mxArray * prhs[] );

  if(sf_Force_Rendering_process_check_sum_call(nlhs,plhs,nrhs,prhs)) return 1;
  return 0;
}
unsigned int sf_process_autoinheritence_call( int nlhs, mxArray * plhs[], int
 nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_Force_Rendering_autoinheritance_info( int nlhs, mxArray
   * plhs[], int nrhs, const mxArray * prhs[] );
  if(sf_Force_Rendering_autoinheritance_info(nlhs,plhs,nrhs,prhs)) return 1;
  return 0;
}
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray
 * plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_Force_Rendering_get_eml_resolved_functions_info( int
  ...
  if(strcmp(machineName, "Force_Rendering") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return
    sf_Force_Rendering_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_Force_Rendering_method_dispatcher(SimStruct *S, unsigned
...
{
  if(!strcmp(machineName,"Force_Rendering")) {
    return(sf_Force_Rendering_method_dispatcher(simstructPtr,chartFileNumber,method,data));
  }
  return 0;
}
extern void Force_Rendering_terminator(void);
void sf_machine_global_terminator(void)
{
  if(sfMachineGlobalTerminatorCallable) {
    ...
    Force_Rendering_terminator();
    sf_debug_terminate();
  }
  return;
}
extern void Force_Rendering_initializer(void);
extern void Force_Rendering_register_exported_symbols(SimStruct* S);
extern void Force_Rendering_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  Force_Rendering_register_exported_symbols(S);
}
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if(sfMachineGlobalInitializerCallable) {
    ...
    if(!simModeIsRTWGen) {
      Force_Rendering_debug_initialize();
    }
    Force_Rendering_initializer();
  }
  return;
}
