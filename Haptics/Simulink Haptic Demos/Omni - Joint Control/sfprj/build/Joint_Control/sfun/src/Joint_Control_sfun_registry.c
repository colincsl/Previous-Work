...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
 const mxArray * prhs[] )
{
  extern unsigned int sf_Joint_Control_process_check_sum_call( int nlhs, mxArray
   * plhs[], int nrhs, const mxArray * prhs[] );

  if(sf_Joint_Control_process_check_sum_call(nlhs,plhs,nrhs,prhs)) return 1;
  return 0;
}
unsigned int sf_process_autoinheritence_call( int nlhs, mxArray * plhs[], int
 nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_Joint_Control_autoinheritance_info( int nlhs, mxArray *
   plhs[], int nrhs, const mxArray * prhs[] );
  if(sf_Joint_Control_autoinheritance_info(nlhs,plhs,nrhs,prhs)) return 1;
  return 0;
}
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray
 * plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_Joint_Control_get_eml_resolved_functions_info( int
  ...
  if(strcmp(machineName, "Joint_Control") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_Joint_Control_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_Joint_Control_method_dispatcher(SimStruct *S, unsigned
...
{
  if(!strcmp(machineName,"Joint_Control")) {
    return(sf_Joint_Control_method_dispatcher(simstructPtr,chartFileNumber,method,data));
  }
  return 0;
}
extern void Joint_Control_terminator(void);
void sf_machine_global_terminator(void)
{
  if(sfMachineGlobalTerminatorCallable) {
    ...
    Joint_Control_terminator();
    sf_debug_terminate();
  }
  return;
}
extern void Joint_Control_initializer(void);
extern void Joint_Control_register_exported_symbols(SimStruct* S);
extern void Joint_Control_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  Joint_Control_register_exported_symbols(S);
}
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if(sfMachineGlobalInitializerCallable) {
    ...
    if(!simModeIsRTWGen) {
      Joint_Control_debug_initialize();
    }
    Joint_Control_initializer();
  }
  return;
}
