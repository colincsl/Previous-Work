...
typedef Joint_Control_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
extern RT_MODEL * Joint_Control(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Joint_Control_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://Joint_Control:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Joint_Control -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(Joint_Control_DWork.HILReadEncoderTimebase_Task,
        Joint_Control_P.HILReadEncoderTimebase_Clock, 1000.0, -1)
        ;
      if (result == 0) {
        ...
        /* disarm the timebase */
        hil_task_stop(Joint_Control_DWork.HILReadEncoderTimebase_Task)
          ;
