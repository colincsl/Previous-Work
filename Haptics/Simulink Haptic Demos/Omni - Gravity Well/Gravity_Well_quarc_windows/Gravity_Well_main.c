...
typedef Gravity_Well_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
extern RT_MODEL * Gravity_Well(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Gravity_Well_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://Gravity_Well:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Gravity_Well -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      if (subrate_index < 0) {         /* all threads successfully created */
        ...
        fflush(stdout);
        result = hil_task_start(Gravity_Well_DWork.HILReadEncoderTimebase_Task,
          Gravity_Well_P.HILReadEncoderTimebase_Clock, 1000.0, -1)
          ;
        if (result == 0) {
          ...
          /* disarm the timebase */
          hil_task_stop(Gravity_Well_DWork.HILReadEncoderTimebase_Task)
            ;
